...
# Se dice que los objetos cuyo valor puede cambiar son mutables; los objetos cuyo valor no se puede cambiar una vez que se crean se llaman immutable. 
# La mutabilidad de un objeto está determinada por su tipo; por ejemplo, los números, las cadenas y las tuplas son inmutables, 
...
# Así mismo la instanciación de objetos equivaldría a la declaración de variables, y el tiempo de vida de un objeto al ámbito de una variable.


# Los objetos también son conocidos como entidades, cada uno de estos tienen rasgos que los hacen diferentes unos de otros.
# A grandes rasgos podemos definir a un objeto como una entidad que posee características que lo hacen diferente a otros y que reaccionan a eventos. 
...
# Métodos: Son bloques de código (o funciones) de una clase que se utilizan para definir el comportamiento de los objetos.
...
# El método __init__ sirve para realizar cualquier proceso de inicialización que sea necesario como puede ser la asignación de valores a atributos, cálculos, llamadas a otras funciones o procedimientos
# El método __init__, se ejecuta justo después de crear un nuevo objeto a partir de la clase proceso que se conoce con el nombre de instanciación.
# El método __init__, no retorna ningún valor y es opcional, ya que podemos utilizar otro nombre de método para la inicialización de la clase.
# Este método permitirá asignar atributos cada vez que creemos un objeto a partir de esa clase haciéndolo obligatorio. 
...
# En este caso persona1 y persona2 son objetos cuya clase es Persona. Ambos objetos pueden hablar, correr y caminar porque su clase define estas operaciones
# Además poseen un nombre y edad en ambos casos, pero difieren de acuerdo a cada objeto.

# Si vemos los métodos son similares a las funciones, la única diferencia sintáctica entre la definición de un método y la definición de una función es
# que el primer parámetro del método por convención debe ser el nombre self.


# Tanto para acceder a los atributos como para llamar a los métodos se utiliza el método denominado de notación de punto
# que se basa en escribir el nombre del objeto o de la clase seguido de un punto y el nombre del atributo o del método con
...
# Ambos objetos pueden acelerar y frenar, porque su clase define estas operaciones y tienen un color, porque la clase Coche también define ese atributo. 
...
# Un objeto los crea del mismo modo en que se crean las variables en Python, es decir, cuando les asigna un valor por primera vez.
# Si retomamos el ejemplo anterior podemos agregar el número de velocidades de la caja del coche agregando las siguientes líneas a la definición de la clase Coche:

coche_uno.caja_velocidades = 6
print(coche_uno.caja_velocidades)

# Si tratamos de aplicar ese atributo al objeto coche_dos obtendremos un error: AttributeError: 'Coche' object has no attribute 'caja_velocidades'
print(coche_dos.caja_velocidades)

# El objeto coche_dos intenta referenciar al mismo atributo, pero como no está definido en la clase y tampoco lo ha inicializado, el intérprete lanza un error.
...
# En el siguiente ejemplo, el atributo ruedas es un atributo de clase ya que se comparte entre todas las instancias de la clase
# Los atributos color, aceleracion y velocidad en cambio son atributos de instancia ya que su valor es único para cada objeto.
...
class Supervisor(Persona):
    ...
    def __init__(self,dni,nombre,apellido,telefono,categoria):
...
# Tomando en cuenta lo que indica el Diagrama de clases, la clase "Cocodrilo" deriva de dos clases base, la clase "Terrestre" y la clase "Acuatico"
# En el caso que tengamos que implementar Herencia Múltiple en Python debemos indicar en la definición cuales son las clases de las que deriva, 
# y es por eso que entre los paréntesis colocamos las clases "Terrestre" y "Acuatico". 
# Así como en el caso anterior de Herencia simple, la clase Cocodrilo hereda todos los atributos y métodos de las clases  "Terrestre" y "Acuatico".
...
# En el caso en el que tengamos dos métodos con el mismo nombre incluidos en las clases base, como es el caso del método desplazar()
...
# Si no se encuentra, la búsqueda continúa en clases primarias en profundidad, de izquierda a derecha, sin buscar la misma clase dos veces. 
...
# Comprobamos que se ejecuta el método desplazar() de la clase "Terrestre" porque en la definición de la clase Cocodrilo esta definida más a la izquierda.
...
# Como ejemplo vamos a implementar las clases del siguiente diagrama: https://bit.ly/30Zuo31
# Tomaremos la clase "Figura" como clase base, la clase "Poligono" como clase derivada de primer nivel 
# y las clases "Triangulo", "Cuadrado" y "Pentagono" como clases derivadas de segundo nivel. 
...
# Poligono se deriva de Figura , y Triangulo se deriva de Poligono por lo que Triangulo tendra los métodos y atributos 
# tanto de las clase Poligono como de la clase Figura. 
# Si en la clase Poligono alguno de las métodos de Figura es redefinido Triangulo heredara el método redefinido.
...
# En una clase podemos aplicar polimorfismo al implementar dos métodos distintos en implementación pero con el mismo nombre. 
# Python al ser de tipado dinámico, no impone restricciones a los tipos que se le pueden pasar a una función, por ejemplo, más allá de que el objeto se comporte como se espera. 
# Es por ello que encontramos funciones que aplican este concepto y si bien usan un mismo nombre los resultados se adaptan a los datos que reciben como parámetros.
...
# Un objeto de una clase derivada es al mismo tiempo un objeto de la clase padre, de forma que allí donde se requiere un objeto de la clase padre también se puede utilizar uno de la clase hija.
...
# Overriding Methods, le permite sustituir un método proveniente de la Clase Base, en la Clase Derivada debe definir un método con la misma forma 
# (es decir, mismo nombre de método y mismo número de parámetros que como está definido en la Clase Base).
# En Python no existe sobrecarga de métodos (el último método sobreescribiría la implementación de los anteriores), aunque se puede conseguir un comportamiento similar recurriendo 
# a funciones con valores por defecto para los parámetros o a la sintaxis *params o **params , o bien usando decoradores.
